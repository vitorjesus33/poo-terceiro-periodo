...
public class Menu extends App {
    ...
    public static void finalizaVenda(List<Produto> listaDeProdutosVendidos) {
        ...
        String conteudo = valorTotalCompra + ";" + totalDePordutos + ";" + (descontoAplicado? "sim" : "não");
        HistoricoDeVendas.gravaArquivo(conteudo, HistoricoDeVendas.getCaminho() + "historicodevendas.csv");
    ...
    public static void finalizaSistema(int quantidade, double valorTotal, double valorInseridoPeloUsuario) {
        ...
        HistoricoDeVendas.gravaTotalDeVendasDoDia(conteudo, HistoricoDeVendas.getCaminho() + localDoArquivo);
        entrada.nextLine();
    }
    ...
    public static void exibeHistorico() {
        ...
        switch (menu) {
            case 1:
                HistoricoDeVendas.lerArquivo(HistoricoDeVendas.getCaminho() + "historicodevendas.csv");
                break;
            case 2:
                HistoricoDeVendas.lerArquivo(HistoricoDeVendas.getCaminho() + "relatoriodiario.csv");
                break;
            case 0:
    ...
    public static void buscarPorDiaExpecifico() {
        ...
        switch (menu) {
            case 1:
                ...
                HistoricoDeVendas.ProcurarNoArquivo(HistoricoDeVendas.getCaminho() + "historicodevendas.csv", busca);
                break;
            case 2:
                System.out.println("digite qual dia ou mês está procurando Exemplo:(26-08-2024 10:49)");
                busca = entrada.nextLine();
                HistoricoDeVendas.ProcurarNoArquivo(HistoricoDeVendas.getCaminho() + "relatoriodiario.csv", busca);
                break;
            case 0:
                System.out.println("retornado ao menu");
                return;
